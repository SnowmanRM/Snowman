...
class ConfigPatterns:
	...
	EVENT_FILTER = r"(?:threshold|event_filter)\s*gen_id\s*(\d+)\s*,\s*sig_id\s*(\d+)\s*,\s*type\s*(.*?)\s*,\s*track\s*(.*?)\s*,\s*count\s*(\d*)\s*,\s*seconds\s*(\d*)"
	DETECTION_FILTER = r".*detection_filter:\s*track\s*(.*?)\s*,\s*count\s*(\d*)\s*,\s*seconds\s*(\d*)\s*;"
	SUPPRESS = r"suppress\s*gen_id\s*(\d+)\s*,\s*sig_id\s*(\d+)(?=\s*,\s*track\s*(.*?)\s*,\s*ip\s*(.*))?"

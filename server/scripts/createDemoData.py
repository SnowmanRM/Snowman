...
a, created = Source.objects.get_or_create(name = "OSourcefire", url="http://filer.rothaugane.com/rules/sf.zip", md5url="http://filer.rothaugane.com/rules/sf.zip.md5", schedule="0 1 * * *")
if(created):
	logger.info("Created '%s'" % a)
else:
	logger.info("'%s' already exists.", a)
